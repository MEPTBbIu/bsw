...
const ProgressBar = ({ progress }) => (
	<div className="progress-container">
		<div className="progress-bar" style={ { transform: `scaleX(${Math.max(0, Math.min(1, progress))})`, }}/>
	</div>
);

const colors = ['#209D22', '#106CCC', '#C1146B', '#11BDBF', '#8A19EA'];
const ProgressPage = ({ view, index, onClick }) => (
	<AnimatedView key={ index} index={ index}
	...
	render() {
		...
		return (
			<ViewPager className="viewport">
				<Frame ref={c => this.frame = c}
						className="frame">
					<Track currentView={currentView} onScroll={this._handleScroll}
							...
							onViewChange={
currentIndicies => { this.setState({ currentView: currentIndicies[0] }) }}
							className="track">
						{views.map((view, index) => <ProgressView key={`page-${index}`} children={view}/>)}
					</Track>
				</Frame>
		...
		return (
			<div>
				<div>
					...
					<input type="range" min={ 0} max={ 3} value={+activeIndex} onChange={ e => this.setState({
					...
					<input type="range" min={1} max={ 3} value={ +viewsToShow} onChange={ e => this.setState({
				...
				<ViewPager>
					<Frame autoSize="height" className="frame">
						<Track ref={ c => this.slider = c} currentView={ activeIndex} viewsToShow={ viewsToShow}
								// viewsToMove={2}
								...
								contain
								onViewChange={ currentIndicies => { this.setState({ activeIndex: currentIndicies[0] }) }}
